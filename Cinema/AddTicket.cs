    ...
    public partial class AddTicket : Form
    {
        ...
        private void AddRec_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ticket.Add(NameVisitor.Text, Convert.ToInt32(Place.Text), Convert.ToInt32(price.Text), id_movie, Convert.ToInt32(Hall.Text), dateTimePicker1.Value.Date);
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            var Conection = new NpgsqlConnection(str.Con);
            Conection.Open();
            var command = new NpgsqlCommand(String.Format("SELECT \"IDMovie\" FROM \"Movies\" WHERE \"NameMovie\"='{0}'", sel_movie), Conection);
            NpgsqlDataReader reader = command.ExecuteReader();
            reader.Read();
            id_movie = (int)reader.GetValue(0);
            reader.Close();
            command = new NpgsqlCommand(String.Format("SELECT \"UsingHall\", \"PriceMovie\" FROM \"Session\" WHERE \"WhatMovie\"={0}", id_movie), Conection);
            ...
            Conection.Open();
            command = new NpgsqlCommand(String.Format("SELECT \"Place\" FROM \"Tickets\" WHERE \"VisitorMovie\"={0}", id_movie), Conection);
            reader = command.ExecuteReader();
            while (reader.Read())
